...
5. Follow the [mobile visual QA workflow](../design/visual-qa-workflow.md). Capture Android and iOS beside the authoritative design at matching representative phone viewports; use overlays when useful. Compare hierarchy, controls, copy, states, responsiveness, keyboard/safe-area behavior, and overall visual character. Do not copy fixed preview coordinates at the expense of correct native responsiveness.
...
7. Capture comparison screenshots and concise reproduction steps for failures. Store evidence only in the ticket-approved artifact location; do not add large generated evidence to source control unless instructed.
