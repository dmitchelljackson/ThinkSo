...
The reviewer must read every changed file's complete diff and full head file, reading lockfile and generated-artifact diffs against their sources, and returns a per-file coverage ledger. The parent refuses to post unless every changed file is covered, and it supplies each file's commentable line ranges so findings anchor to valid diff lines. The reviewer posts one normal review with a short summary and inline findings. Previous automated review text is deliberately excluded from model context so each head receives an independent review, while the trusted parent reads prior App-authored inline IDs and supplies the next sequential `CR-###` number. Duplicate-run markers count only when authored by the verified App bot identity. Optional `why` evidence is validated against the injected knowledge and rendered as a permanent rule link.
...
- `review.py` — fetches one PR, injects one complete bounded-context diff and per-file commentable line ranges without duplicating GitHub patches, launches the reviewer, validates its result and file coverage, and posts through the App.
